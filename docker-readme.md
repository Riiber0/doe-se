...
```bash
dk bash  # starts bash inside "doese" container
...
* `dkup` uses docker-compose to start 3 containers: postgres, nginx, and doese.
...
* Once `dkup` is running, `dk <command>` will run `<command>` inside the `doese` container. So `dk bash` will get you "logged in" as root inside that container. Once inside, you need to run Django's `manage.py` commands to initialize the database properly.
* The doese container runs 3 services:
 * django on port 8000
...
```bash
mkvirtualenv doese -p python3  # creates a python3 virtualenv
